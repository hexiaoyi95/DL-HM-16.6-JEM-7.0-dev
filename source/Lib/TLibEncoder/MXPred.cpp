...
void GetImageFile(const std::string image_file,
    ...
    cv::Mat im_ori = cv::imread(image_file, cv::IMREAD_GRAYSCALE);
    ...
    std::cout<< "input size:" << im.rows <<"," << im.cols << ","  <<  channels << std::endl;
    mx_float* ptr_image_1 = image_data;
    mx_float* ptr_image_2 = image_data + size/2;
    ...
    for (int i = 0; i < im.rows; i++) {
        ...
        for (int j = 0; j < im.cols; j++) {
            ...
            *ptr_image_1++ = static_cast<mx_float>(*data++) /255;
            *ptr_image_2++ = *ptr_image_1;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    std::string json_file = "/home/l301/mxnet/tools/caffe_converter/vrcnn_recon+pred_concat-symbol.json";
    std::string param_file = "/home/l301/mxnet/tools/caffe_converter/vrcnn_recon+pred_concat-0000.params";
   ...
   // const char* input_key2 = "data2";
    const char** input_keys = input_key;
    std::cout << input_keys[0] << std::endl;
    //std::cout << input_keys[1] << std::endl;
    // Image size and channels
    int width = 416;
    int height = 240;
    int channels = 2;

    const mx_uint input_shape_indptr[4] = { 0, 4 };
    ...
    MXPredCreate((const char*)json_data.GetBuffer(),
                 ...
                 &pred_hnd);
    assert(pred_hnd);
    std::cout << "create net successed" << std::endl;
   ...
   // }
    ...
    cv::Mat temp1(height, width, CV_32FC1 , &(image_data[0]));
    cv::Mat input_uint8;
    temp1.convertTo(input_uint8, CV_8UC1, 255);
    imwrite("input.bmp", input_uint8);
    imshow("input",input_uint8);
     
    // Set Input Image
    MXPredSetInput(pred_hnd, "data", image_data.data(), image_size);
    //MXPredSetInput(pred_hnd, input_keys[1], image_data2.data(), image_size);
    std::cout << "set input successed" << std::endl;    
    // Do Predict Forward
    MXPredForward(pred_hnd);
    std::cout << "forward successed" << std::endl;
    ...
    cv::Mat temp(shape[3] , shape[2], CV_32FC1 , &(data[0]));
    cv::Mat pred_uint8;
    temp.convertTo(pred_uint8, CV_8UC1, 255);
    imwrite("output.bmp", pred_uint8);
    imshow("output",pred_uint8);
    cv::waitKey();
    // Release NDList
    //if (nd_hnd)
    //  MXNDListFree(nd_hnd);
